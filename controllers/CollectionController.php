...
class CollectionController extends Controller {
  ...
  public function showTag(Request $request, Response $response) {
    $params = $request->getQueryParams();
		$count_product = 0;
    $tag_handle = $request->getAttribute('handle');
    $tag = CollectionTag::where('handle', $tag_handle)->first();
    if(!$tag) {
      $this->view->render($response, '404.pug');
      return $response->withStatus(404);
    }
    ...
    $temp->title = 'Tag';
    $temp->handle = '';
    array_push($breadcrumb_collection, $temp);
		...
		$obj = new stdClass();
		$obj->page_type = 'tag';
		$obj->tag_handle = $tag_handle;
		$link = $_SERVER['REQUEST_URI'];
		if(strrpos($link, '?') !== false) {
			$link = substr($link, strrpos($link, '?') + 1);
			$link = explode('&', $link);
			foreach ($link as $key => $value) {
				if(strpos($value, 'brand=') > -1) {
					$obj->brand = str_replace('brand=', '', $value);
				}
			}
		}
		$obj->color = $params['color'];
		$obj->price = $params['price'];
		$obj->sort = $params['orderBy'];
		$arr_filter = Product::arrayFilter($obj);
		$query = Product::checkFilter($obj);
    // $count_products = count($query->groupBy('product.group_id')->get());
    $count_products = count($query->get());
		$orderBy = $params['orderBy'];
		if($orderBy) {
			$arr_orderBy = explode(':', $orderBy);
			$query = $query->orderBy('product.'.$arr_orderBy[0], $arr_orderBy[1]);
		} else $query = $query->orderBy('product.in_stock', 'desc')->orderBy('product.updated_at', 'desc');
		// $products = $query->skip($skip)->take($perpage)->groupBy('product.group_id')->get();
    $products = $query->skip($skip)->take($perpage)->get();
		$total_pages = ceil($count_products / $perpage);
		$products = Product::getInfoProduct($products);
		$list_brand = array();
		$list_color = array();
		$arr_temp_color = array();
    // $all_products = Product::Join('collection_product', 'collection_product.product_id', '=', 'product.id')->join('collection_tag', 'collection_tag.collection_id', '=', 'collection_product.collection_id')->where('collection_tag.handle', $tag_handle)
		// 	->where('product.display', 1)->select('product.id as id', 'product.brand as brand')->groupBy('product.group_id')->get();
    $all_products = Product::Join('collection_product', 'collection_product.product_id', '=', 'product.id')->join('collection_tag', 'collection_tag.collection_id', '=', 'collection_product.collection_id')->where('collection_tag.handle', $tag_handle)
			->where('product.display', 1)->select('product.id as id', 'product.brand as brand')->get();
		foreach ($all_products as $key => $product) {
			if(!in_array($product->brand, $list_brand)) array_push($list_brand, $product->brand);
			$product_color = ProductColor::Join('color', 'product_color.color_id', '=', 'color.id')
				->where('product_color.product_id', $product->id)->select('color.name as name', 'color.hex as hex')->get();
			foreach ($product_color as $key => $color) {
				if(!in_array($color->name, $arr_temp_color)) {
		...
		$count_brand_filter = count($list_brand);
		$count_color_filter = count($list_color);
    $collection->product = $products;
    $this->view->render($response, 'collection.pug', array(
			'title' => $tag->name,
			'data' => $collection,
      'list_product' => $products,
			'total_pages' => $total_pages,
			...
			'list_brand' => $list_brand,
			'list_color' => $list_color,
			'count_brand_filter' => $count_brand_filter,
			'count_color_filter' => $count_color_filter,
			'breadcrumb_title' => $breadcrumb_title,
			'breadcrumb_collection' => $breadcrumb_collection,
